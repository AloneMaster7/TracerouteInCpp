...
int main(){
    ...
    for(int ttl=1;ttl<256;ttl++){
        ...
        //timeout.tv_sec = 0;
        //timeout.tv_usec = 1000+rand()%500000;
        if(setsockopt(udp,IPPROTO_IP,IP_TTL,&ttl,sizeof(ttl))){
            cout<<"ttl sockopt err"<<endl;
            return -1;
        }
        memset(SendBuf,0,sizeof(SendBuf));
        short* i = (short*)SendBuf;*i = seq++;
        i++;*i=ttl;
        i++;timeval* tv = (timeval*)i;gettimeofday(tv,nullptr);
        char* p = (char*)(tv+1);
        memset(p,0xab,20);
        int bytes = sendto(udp,"a",1
                           /*SendBuf,32*//*sizeof(short)*2+sizeof(timeval)*/,0,
